...
public class AspectFrameLayout extends FrameLayout
{
    ...
    private double targetRatio = -1.0;        // initially use default window size
    ...
    public void setAspectRatio(double aspectRatio)
    {
        ...
        Log.d(TAG, "Setting aspect ratio to " + aspectRatio + " (was " + targetRatio + ")");
    ...
    {
        Log.d(TAG, "onMeasure target=" + targetRatio +
        ...
        if (targetRatio > 0) {
            ...
            double aspectDiff = targetRatio / viewAspectRatio - 1;
            ...
            if (Math.abs(aspectDiff) < 0.01) {
                ...
                Log.d(TAG, "aspect ratio is good (target=" + targetRatio +
                        ", view=" + initialWidth + "x" + initialHeight + ")");
            } else {
                if (aspectDiff > 0) {
                    // limited by narrow width; restrict height
                    initialHeight = (int) (initialWidth / targetRatio);
                } else {
                    // limited by short height; restrict width
                    initialWidth = (int) (initialHeight * targetRatio);
                }
                Log.d(TAG, "new size=" + initialWidth + "x" + initialHeight + " + padding " +
                        horizontalPadding + "x" + verticalPadding);
                initialWidth += horizontalPadding;
                initialHeight += verticalPadding;
